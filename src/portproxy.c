...
int sockets_bind(context_t * ctx)
{
	...
	result = socket_add(ctx, ctx->lan, ctx->lanIP, ctx->upnp_tcp_port1, IPPROTO_TCP, 0/*don't bind to iface*/, 0/*don't allow reuse*/);
	port = sock_get_port(result);
	Log(2,"TCP: sockid=%d, port=%d\n", result, port);
	if (result<0)
	    return -1;
	
	result = socket_add(ctx, ctx->lan, ctx->lanIP, ctx->upnp_tcp_port2, IPPROTO_TCP, 0/*don't bind to iface*/, 0/*don't allow reuse*/);
	port = sock_get_port(result);
	Log(2,"TCP: sockid=%d, port=%d\n", result, port);
	if (result<0)
	    return -1;
